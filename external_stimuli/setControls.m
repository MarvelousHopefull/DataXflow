...
if(isempty(A)==1)                               %Test if A is not empty, if empty return u as a zero matrix, that means no external stimulus is active
    ...
    for i=1:OCP.numControls                     %Loop over all external stimuli
        ...
        if(is_in==1)                            %Construct external stimulus i as a function over time
