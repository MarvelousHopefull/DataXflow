...
%u initial guess for the external stimuli, can be taken from the combinatorial method, details like format see main.m Variables 
...
sigma=50;                                               %Parameter for enlarging epsilon
zeta=0.15;                                              %Parameter for diminishing epsilon
eta=10^-12;                                             %Parameter for comparing the values of the target functional for new and old u
valueTargetFunctional(1)=J(u,x,xd,OCP);                 %Calculates value of target functional for the initial guess of u
...
while(1==1)
    for j=1:Nt                                                                                      %For loop over time steps
        ...
        eval_deriv=sparse(reshape(eval_deriv,[M,N])');                                              %Reshapes vector to a matrix and transform it into a sparse one
    ...
    if (J_int-valueTargetFunctional(counter)>-eta*du)    %Checks if the update on u with corresponding x induces at least -eta*du descent for the target functional
        ...
        counter=counter+1;                          
        epsilon=epsilon*zeta;                            %Diminish epsilon
        u_old=u;                                         %Adopt u
        x=x_int;                                         %Adopt x
