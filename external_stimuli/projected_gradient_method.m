...
%Input:grad function handle for the gradient of the reduced target functional J(u):=J(x(u),u), proj function handle of the projection, projects u into [0,1] 
        ...
        %u initial guess for the external stimuli, can be taken from the combinatorial method, details like format see main Variables 
...
beta2=1.1;                                          %Parameter for enlarging the step size
dt=OCP.timeInterval;
...
valueTargetFunctional(1,1)=J(u,f,xd,OCP);           %value of the target functional with the initial guess of the external stimuli u
...
while(res>tol3 && count<max_iter)                   %Perform the projected gradient method as long as the residuum of the projected gradient is greater than tol2 or the number of iterations is less than max_iter
    ...
    fprintf('Iteration %i, value target functional J=%d with residuum res=%d and step size t=%d\n',count,valueTargetFunctional(1,count+1),res,t);
